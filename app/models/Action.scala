      ...
      actionType.fold(
        errors => JsError("actionType undefined or incorrect"),
        {
          ...
          case "UpdateConfigAction" =>
            (JsPath \ "action").read[UpdateConfigAction].reads(js)
        }
      )
    },
    Writes {
      case b: SendFundsAction =>
        JsObject(
          Seq(
      ...
      case b: UpdateConfigAction =>
        JsObject(
          Seq(
            "actionType" -> JsString("UpdateConfigAction"),
            "action" -> UpdateConfigAction.format.writes(b)
          )
        )
    }
  )
